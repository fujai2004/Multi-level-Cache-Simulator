...
twoleveltests_json = """{
"2lvlt1": {
      "diff <(./model/2level-mutex-ref -c ./model/2level_1.config  -t ./model/traces/fifo_s2.trace) <(./model/2level-mutex -c ./model/2level_1.config -t ./model/traces/fifo_s2.trace)": 10
    },
"2lvlt2": {
      "diff <(./model/2level-mutex-ref  -c ./model/2level_2.config -t ./model/traces/long.trace) <(./model/2level-mutex -c ./model/2level_2.config -t ./model/traces/long.trace)": 10
    },
"2lvlt3": {
      "diff <(./model/2level-mutex-ref -L -c ./model/2level_2.config -t ./model/traces/long.trace) <(./model/2level-mutex -L -c ./model/2level_2.config -t ./model/traces/long.trace)": 10
    },
"2lvlt4": {
      "diff <(./model/2level-mutex-ref -L -c ./model/2level_3.config -t ./model/traces/long.trace) <(./model/2level-mutex -L -c ./model/2level_3.config -t ./model/traces/long.trace)": 10
    }   
    }"""
...
def getfields(out_lines):
    ...
    for lines in out_lines:
        if b"Cache block size" in lines:
            block = int(re.findall(r'\d+', lines.decode('ISO-8859-1'))[0])
        elif b"Cache size" in lines:
            size = int(re.findall(r'\d+', lines.decode('ISO-8859-1'))[0])
        elif b"Cache associativity" in lines:
            assoc = int(re.findall(r'\d+', lines.decode('ISO-8859-1'))[0])
    ...
    for steps in test.keys():
        ...
        for lines in out_lines:
            if b"AUTORESULT_STRING=" in lines:
                points += int(re.findall(r'\d+', lines.decode('ISO-8859-1'))[0])
                print(int(re.findall(r'\d+', lines.decode('ISO-8859-1'))[0]))
        if (p.returncode == 0):
            Success += "#### " + "*"*5+steps+"*"*5
            Success += "\n ```" + stdout_data.decode('ISO-8859-1') + "\n```\n"
            points += test[steps]
